...
- OpenAI, Using skills in Codex
  - https://developers.openai.com/codex/skills

### 代表性开源仓库

- woshiheihei/baoyu-skills
  - https://github.com/woshiheihei/baoyu-skills
...
### 4.1 安装体验未必要靠自定义脚本，轻量说明常常更优

从公开仓库的分发方式看，安装并没有单一标准，但有一个很稳定的趋势：如果宿主 agent 已经有原生安装能力，那么仓库作者往往只需要把 README 写清楚，而不是再维护一层自定义 shell 脚本。

实践观察：

- `openai/skills` 直接让用户使用 Codex 内置的 `$skill-installer`
- Codex 官方文档明确支持仓库内 `.agents/skills/` 自动发现，甚至支持 symlinked skill folders
- `woshiheihei/baoyu-skills` 把 “Ask your agents” 作为正式安装选项之一
- 很多 skill 仓库把“仓库目录即 source of truth”，安装只是把该目录同步到 agent 可发现的位置

对本仓库的启发：

- 仓库内应提供 `.agents/skills/` 入口，降低本地试用门槛
- README 应优先提供 “Ask your agent” 方式
- 对 Codex 用户，README 可直接给出 `$skill-installer install <GitHub 目录 URL>`
- 不必为了一个很薄的复制动作额外维护平台相关安装脚本
